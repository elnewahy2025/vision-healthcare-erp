    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-[var(--text-primary)]">{t('kiosk.title')}</h1>
          <p className="text-[var(--text-muted)] mt-2">{t('kiosk.subtitle')}</p>
        </div>
        ...
        {step === 'enter' && (
          ...
          <Button
                ...
                <p className="text-center text-sm text-[var(--text-disabled)]">
                  {t('kiosk.helpHint')}
                </p>
              </div>
            </CardBody>
        ...
        {step === 'success' && result && (
          <Card>
            <CardBody className="p-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-2">{t('kiosk.checkedIn')}</h2>
              <p className="text-[var(--text-secondary)] mb-6">
                {t('kiosk.welcome', { name: sanitizeString(result.patientName) })}
              </p>
              <div className="bg-blue-50 rounded-xl p-6 mb-6">
                <p className="text-sm text-[var(--text-muted)] mb-1">{t('kiosk.yourQueueNumber')}</p>
          ...
          <Card>
            <CardBody className="p-8">
              ...
              <div className="space-y-4">
                <div className="flex justify-between p-3 bg-[var(--surface-secondary)] rounded-lg">
                  <span className="text-[var(--text-secondary)]">{t('kiosk.yourNumber')}</span>
                  <span className="font-bold text-lg">#{status.queue_number}</span>
                </div>
                <div className="flex justify-between p-3 bg-[var(--surface-secondary)] rounded-lg">
                  <span className="text-[var(--text-secondary)]">{t('kiosk.status')}</span>
