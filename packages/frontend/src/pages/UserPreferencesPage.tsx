          ...
          <CardBody>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  >
                    <opt.icon className={`w-6 h-6 mx-auto mb-2 ${theme === opt.value ? 'text-primary-600' : 'text-[var(--text-disabled)]'}`} />
            ...
            <div>
              ...
              <div className="flex items-center gap-3 p-3 bg-[var(--surface-secondary)] rounded-lg">
                ...
                <span className="text-sm text-[var(--text-secondary)]">{t('preferences.quickSearchHint')}</span>
              </div>
            </div>
          </CardBody>
        </Card>
              ...
              notifPrefs.map((p) => (
                <div key={p.id} className="flex items-center justify-between py-3 border-b last:border-0">
                  <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)]">{p.events.join(', ')}</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    />
                    <div className="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-primary-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-[var(--surface)] after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary-600" />
                  </label>
                </div>
              ))
            )}
                ...
                {shortcuts.map((s, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b last:border-0">
                    <div>
                      ...
                      <p className="text-xs text-[var(--text-muted)] capitalize">{categoryLabels[s.category] ?? sanitizeString(s.category)}</p>
                    </div>
                    <kbd className="px-2 py-1 bg-[var(--surface-hover)] border rounded text-xs font-mono">{sanitizeString(s.key)}</kbd>
                  </div>
                ))}
              </div>
            )}
