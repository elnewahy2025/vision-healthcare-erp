...
const typeColor: Record<string, string> = { emr: 'bg-blue-100 text-blue-600', appointment: 'bg-green-100 text-green-600', invoice: 'bg-purple-100 text-purple-600', document: 'bg-[var(--surface-hover)] text-[var(--text-secondary)]', allergy: 'bg-red-100 text-red-600' };
...
export default function PatientTimelinePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {timeline.length === 0 ? (
        <div className="text-center py-12 text-[var(--text-muted)]">No activity recorded yet</div>
      ) : (
        <div className="relative">
          ...
          <div className="space-y-4">
            {timeline.map((event: TimelineEvent, i: number) => {
              ...
              const color = typeColor[event.type] || 'bg-[var(--surface-hover)] text-[var(--text-secondary)]';
              return (
                <div key={`${event.type}-${event.id}-${i}`} className="relative flex gap-4 pl-0">
                  ...
                  <div className="flex-1 bg-[var(--surface)] rounded-xl border border-[var(--border)] dark:bg-[var(--background)] dark:border-gray-800 p-4 hover:shadow-sm transition-shadow">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="font-medium text-[var(--text-primary)] capitalize">{event.title || event.type}</p>
                        <p className="text-xs text-[var(--text-muted)] mt-1">{new Date(event.date).toLocaleDateString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}</p>
                      </div>
                    ...
                    {event.diagnosis && <p className="text-xs text-[var(--text-muted)] mt-1 truncate">Dx: {typeof event.diagnosis === 'string' ? event.diagnosis : JSON.stringify(event.diagnosis)}</p>}
                  </div>
                </div>
              );
            })}
