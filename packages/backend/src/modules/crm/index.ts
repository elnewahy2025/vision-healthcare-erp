...
export async function registerCrmModule(app: FastifyInstance) {
  app.get('/api/v1/crm/campaigns', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
    ...
    return sendSuccess(reply, rows.map((c: any) => ({ id: c.id, name: c.name, type: c.type, status: c.status, description: c.description, startDate: c.start_date, endDate: c.end_date, budget: Number(c.budget), targetCount: c.target_count, reachedCount: c.reached_count, conversionCount: c.conversion_count })));
  });

  app.post('/api/v1/crm/campaigns', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
  ...
  app.get('/api/v1/crm/feedback', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
    ...
    return sendSuccess(reply, { averageRating: Number((avg as any)?.avg || 0).toFixed(1), total: rows.length, feedback: rows.map((f: any) => ({ id: f.id, patientId: f.patient_id, patientName: f.pf + ' ' + f.pl, rating: f.rating, comment: f.comment, category: f.category, createdAt: f.created_at })) });
  });
}
