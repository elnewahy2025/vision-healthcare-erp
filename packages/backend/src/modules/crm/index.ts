...
export async function registerCrmModule(app: FastifyInstance) {
  ...
  app.get('/api/v1/crm/feedback', { preHandler: [(r: FastifyRequest, rep: FastifyReply) => authenticate(r, rep)] }, async (request, reply) => {
    ...
    return sendSuccess(reply, { averageRating: Number((avg as any)?.avg || 0).toFixed(1), total: rows.length, feedback: rows.map((f: Record<string, unknown>) => ({ id: f.id, patientId: f.patient_id, patientName: f.pf + ' ' + f.pl, rating: f.rating, comment: f.comment, category: f.category, createdAt: f.created_at })) });
  });
}
