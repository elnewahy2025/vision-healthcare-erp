...
export async function registerInventoryModule(app: FastifyInstance) {
  ...
  app.post('/api/v1/inventory/pos', { preHandler: [(r: FastifyRequest, rep: FastifyReply) => authenticate(r, rep)] }, async (request, reply) => {
    ...
    let total = 0;
    if (body.items) { body.items.forEach((i: any) => { total += (i.quantityOrdered || 0) * (i.unitCost || 0); }); }
    ...
    if (body.items) {
      await db('purchase_order_items').insert(body.items.map((i: any) => ({ po_id: po.id, item_id: i.itemId || null, item_name: i.itemName, sku: i.sku || null, quantity_ordered: i.quantityOrdered, unit_cost: i.unitCost || 0, total_cost: (i.quantityOrdered || 0) * (i.unitCost || 0) })));
    }
