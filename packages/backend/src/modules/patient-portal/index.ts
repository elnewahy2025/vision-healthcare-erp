...
export async function registerPatientPortalModule(app: FastifyInstance) {
  ...
  app.get('/api/v1/portal/dashboard', { preHandler: portalAuth }, async (request, reply) => {
    const patientId = (request as Record<string, unknown>).patientId;
    ...
    return sendSuccess(reply, {
      ...
      upcomingAppointments: upcomingAppts.map((a: PatientMessageRow) => ({ id: a.id, date: a.appointment_date, time: a.start_time, type: a.appointment_type, status: a.status, doctorId: a.doctor_id, branchId: a.branch_id })),
      recentRecords: recentRecords.map((r: PatientMessageRow) => ({ id: r.id, diagnosis: r.diagnosis?.substring(0, 100), createdAt: r.created_at })),
      pendingBills: recentInvoices.map((i: PatientMessageRow) => ({ id: i.id, invoiceNumber: i.invoice_number, total: Number(i.total), dueAmount: Number(i.total) - Number(i.paid), dueDate: i.due_date })),
  ...
  app.get('/api/v1/portal/documents', { preHandler: portalAuth }, async (request, reply) => {
    ...
    return sendSuccess(reply, docs.map((d: InvoiceRow) => ({
  ...
  app.post('/api/v1/portal/logout', { preHandler: portalAuth }, async (request, reply) => {
    await db('portal_sessions').where({ id: (request as Record<string, unknown>).portalSession.id }).del();
