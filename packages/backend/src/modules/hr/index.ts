...
export async function registerHrModule(app: FastifyInstance) {
  app.get('/api/v1/hr/employees', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
    const tenantId = getTenantId(request); const { department, status } = request.query as any;
    ...
    return sendSuccess(reply, emps.map((e: any) => ({ id: e.id, employeeCode: e.employee_code, firstName: e.first_name, lastName: e.last_name, email: e.email, phone: e.phone, department: e.department, position: e.position, employmentType: e.employment_type, hireDate: e.hire_date, status: e.status, baseSalary: Number(e.base_salary) })));
  });

  app.post('/api/v1/hr/employees', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
  ...
  app.get('/api/v1/hr/attendance', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
    const tenantId = getTenantId(request); const { date, employeeId } = request.query as any;
    ...
    return sendSuccess(reply, rows.map((r: any) => ({ id: r.id, employeeId: r.employee_id, employeeName: r.ef + ' ' + r.el, date: r.date, clockIn: r.clock_in, clockOut: r.clock_out, status: r.status, notes: r.notes })));
  });

  app.get('/api/v1/hr/leave-requests', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
    const tenantId = getTenantId(request); const { status, employeeId } = request.query as any;
    ...
    return sendSuccess(reply, rows.map((r: any) => ({ id: r.id, employeeId: r.employee_id, employeeName: r.ef + ' ' + r.el, leaveType: r.leave_type, startDate: r.start_date, endDate: r.end_date, totalDays: r.total_days, status: r.status, reason: r.reason, managerNotes: r.manager_notes, createdAt: r.created_at })));
  });

  app.post('/api/v1/hr/leave-requests', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
    const tenantId = getTenantId(request); const ctx = getCtx(request); const body = request.body as any;
  ...
  app.put('/api/v1/hr/leave-requests/:id/approve', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
    const { id } = request.params as any; const ctx = getCtx(request); const { status, managerNotes } = request.body as any;
