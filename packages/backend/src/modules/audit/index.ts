...
export async function registerAuditModule(app: FastifyInstance) {
  ...
  app.get('/api/v1/audit-logs', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
  ...
  app.get('/api/v1/audit-logs/:id', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
    const { tenantId } = getCtx(request);
  ...
  app.get('/api/v1/audit-logs/actions/types', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
    const { tenantId } = getCtx(request);
  ...
  app.get('/api/v1/audit/logs/export', { preHandler: [(r: any, rep: any) => (r.server as any).authenticate(r, rep)] }, async (request, reply) => {
    const { tenantId } = getCtx(request);
    ...
    if (query.format === 'csv') {
      ...
      const csv = [headers.join(','), ...logs.map((l: any) => headers.map(h => String(l[h] || '').replace(/,/g, ';')).join(','))].join('\n');
