...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!(await knex.schema.hasTable('dw_appointment_stats'))) {
  ...
  if (!(await knex.schema.hasTable('dw_revenue_stats'))) {
  ...
  if (!(await knex.schema.hasTable('dw_patient_stats'))) {
  ...
  if (!(await knex.schema.hasTable('automation_logs'))) {
  ...
  // Barcode/QR Registry
  if (!(await knex.schema.hasTable('barcode_registry'))) {
