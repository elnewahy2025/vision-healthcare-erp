...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!(await knex.schema.hasTable('api_key_logs'))) {
  ...
  // Data Export Engine
  if (!(await knex.schema.hasTable('export_definitions'))) {
  ...
  if (!(await knex.schema.hasTable('export_jobs'))) {
  ...
  // System Monitoring
  if (!(await knex.schema.hasTable('system_metrics'))) {
  ...
  if (!(await knex.schema.hasTable('system_alerts'))) {
  ...
  if (!(await knex.schema.hasTable('cache_configs'))) {
