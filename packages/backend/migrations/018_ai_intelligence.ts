...
export async function up(knex: Knex): Promise<void> {
  // AI Clinical Notes
  if (!(await knex.schema.hasTable('ai_clinical_notes'))) {
  ...
  if (!(await knex.schema.hasTable('ai_diagnosis_suggestions'))) {
  ...
  // AI Predictions
  if (!(await knex.schema.hasTable('ai_predictions'))) {
  ...
  // AI Smart Schedules
  if (!(await knex.schema.hasTable('ai_smart_schedules'))) {
  ...
  // Patient Risk Scores
  if (!(await knex.schema.hasTable('patient_risk_scores'))) {
